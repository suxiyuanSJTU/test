#!/usr/bin/env python
# -*- coding: utf-8 -*-

#ROS/视觉系统/气阀联合
#第一版
#暂时仅考虑正品
#预期功能：将机器人恢复到停泊点，从停泊位置运动到待抓取点。检查视觉系统传输的文件，根据文件内容向气阀发出指令，开启气阀。开启气阀后，下压吸取工件，上提，运动至落料点，关闭气阀。恢复到停泊点。


import rospy, sys
import moveit_commander
from moveit_msgs.msg import RobotTrajectory
from trajectory_msgs.msg import JointTrajectoryPoint

from geometry_msgs.msg import PoseStamped, Pose
from tf.transformations import euler_from_quaternion, quaternion_from_euler
from ctypes import cdll

ARM_GROUP = "manipulator"

class MoveItIkDemo:
    def __init__(self):
        # 初始化move_group的API
        moveit_commander.roscpp_initialize(sys.argv)

        # 初始化ROS节点
        rospy.init_node('abb_moveit_ik_demo')

        # 初始化需要使用move_group控制的机械臂中的arm_group
        arm = moveit_commander.MoveGroupCommander(ARM_GROUP)

        # 获取终端link的名称
        end_effector_link = 'link_6'

        # 设置目标位置所使用的参考坐标系
        reference_frame = 'base_link'
        arm.set_pose_reference_frame(reference_frame)

        # 当运动规划失败后，允许重新规划
        arm.allow_replanning(True)

        # 设置位置（单位：米）和姿态（单位：弧度）的允许误差
        arm.set_goal_position_tolerance(0.01)
        arm.set_goal_orientation_tolerance(0.05)

        # 控制机械臂先回到初始化位置
        arm.set_named_target('all_zero')
        arm.go()
        rospy.sleep(2)

        # joint_positions = [0, 0, 0, 0, 1.571, 0]
        # arm.set_joint_value_target(joint_positions)
        # arm.go()
        # rospy.sleep(1)


        # 设置机械臂工作空间中的目标位姿，即为抓取点，位置使用x,y,z坐标描述
        # 姿态使用四元数描述，基于base_link坐标系，原点为基座底面中央
        target_pose = PoseStamped()
        target_pose.header.frame_id = reference_frame
        target_pose.header.stamp = rospy.Time.now()
        target_pose.pose.position.x = 0.55
        target_pose.pose.position.y = 0
        target_pose.pose.position.z = 0.50
        
        target_pose.pose.orientation.x = 0.5
        target_pose.pose.orientation.y = 0.5
        target_pose.pose.orientation.z = -0.5
        target_pose.pose.orientation.w = 0.5

        # 设置机械臂当前的状态作为运动初始状态
        arm.set_start_state_to_current_state()

        # 设置机械臂终端运动的目标位姿
        arm.set_pose_target(target_pose, end_effector_link)

        # 规划运动路径
        traj = arm.plan()

        print(traj)

        # 按照规划的运动路径控制机械臂运动，运动到抓取点
        arm.execute(traj)
        rospy.sleep(2)
        
        #检查视觉识别端传来的文件
	data = ''
	with open('result.txt','r',encoding='utf=8') as f:
    	    for line in f.readlines():
                line = line.strip()
                data += line
	
	if data == 'y':

            #若为正品，控制机械臂向下移动5cm，压紧工件
            arm.shift_pose_target(2, -0.05, end_effector_link)
            arm.go()
            rospy.sleep(2)


        #向气阀发出控制信号，开启气阀
#————————————————————————————————————————————加载cpp文件————————————————————————————————————————————————	
	#测试，加载编译好的so文件
	    so_lib_path = './c_program.so'

	    so_lib = cdll.LoadLibrary(so_lib_path)
	# 直接用方法名调用，确认成功调用

	    so_lib.display("hello, I am calling a so file.")

#———————————————————————————————————————————————————————————————————————————————————————————————————————

        # 控制机械臂终端向上移动5cm
            arm.shift_pose_target(2, 0.05, end_effector_link)
            arm.go()
            rospy.sleep(2)

        # 令机械臂移动至落料处
	# 中间点1
        # 设置机械臂工作空间中的目标位姿，即为中间点，位置使用x,y,z坐标描述
        # 姿态使用四元数描述，基于base_link坐标系，原点为基座底面中央
            target_pose = PoseStamped()
            target_pose.header.frame_id = reference_frame
            target_pose.header.stamp = rospy.Time.now()
            target_pose.pose.position.x = 0.25
            target_pose.pose.position.y = 0.25
            target_pose.pose.position.z = 0.50
        
            target_pose.pose.orientation.x = 0.5
            target_pose.pose.orientation.y = 0.5
            target_pose.pose.orientation.z = -0.5
            target_pose.pose.orientation.w = 0.5

        # 设置机械臂当前的状态作为运动初始状态
            arm.set_start_state_to_current_state()

        # 设置机械臂终端运动的目标位姿
            arm.set_pose_target(target_pose, end_effector_link)

        # 规划运动路径
            traj = arm.plan()
            print(traj)

        # 按照规划的运动路径控制机械臂运动，运动到中间点
            arm.execute(traj)
            rospy.sleep(2)

        # 设置机械臂工作空间中的目标位姿，即落料点，位置使用x,y,z坐标描述
        # 姿态使用四元数描述，基于base_link坐标系，原点为基座底面中央
            target_pose = PoseStamped()
            target_pose.header.frame_id = reference_frame
            target_pose.header.stamp = rospy.Time.now()
            target_pose.pose.position.x = 0
            target_pose.pose.position.y = 0.5
            target_pose.pose.position.z = 0.5

            target_pose.pose.orientation.x = 0.5
            target_pose.pose.orientation.y = 0.5
            target_pose.pose.orientation.z = -0.5
            target_pose.pose.orientation.w = 0.5

        # 设置机械臂当前的状态作为运动初始状态
            arm.set_start_state_to_current_state()

        # 设置机械臂终端运动的目标位姿
            arm.set_pose_target(target_pose, end_effector_link)

        # 规划运动路径
            traj = arm.plan()

            print(traj)

        # 按照规划的运动路径控制机械臂运动，运动到落料点
            arm.execute(traj)
            rospy.sleep(2)

        # 控制机械臂终端反向旋转90度
        #arm.shift_pose_target(4, -1.57, end_effector_link)
        #arm.go()
        #rospy.sleep(1)

        #向气阀发出信号，关闭气阀
	#测试，加载编译好的so文件
	##   so_lib_path = './c_program2.so'

	  ##  so_lib = cdll.LoadLibrary(so_lib_path)
	# 直接用方法名调用，确认成功调用

	  ##  so_lib.display("hello, I am calling a so file.")


        # 控制机械臂回到初始化位置
            arm.set_named_target('all_zero')
            arm.go()

        # 关闭并退出moveit
            moveit_commander.roscpp_shutdown()
            moveit_commander.os._exit(0)

if __name__ == "__main__":
    MoveItIkDemo()
